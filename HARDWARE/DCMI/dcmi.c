...
u8 finish_flag_034 = 0;

void DCMI_IRQHandler(void)
{
	if(DCMI_GetITStatus(DCMI_IT_VSYNC) == SET)//场中断
	{
		DCMI_ClearITPendingBit(DCMI_IT_VSYNC);//清除场中断
		dcmi_start();
		finish_flag_034 = 1;
	}
} 
...
{ 
	...
	DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;// 使用非循环模式 
	...
	DMA_DoubleBufferModeConfig(DMA2_Stream1, (u32)image + COL * (ROW / 2), (u32)image);
	DMA2_Stream1->M0AR = (u32)image + COL * (ROW / 2);
	DMA2_Stream1->M1AR = (u32)image;
...
{  
	//dcmi_dma_init((u32)image,COL*ROW/2/4,DMA_MemoryDataSize_Word,DMA_MemoryInc_Enable);//DCMI DMA配置
